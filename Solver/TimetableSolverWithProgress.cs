    ...
    public class TimetableSolverWithProgress
    {
        ...
        private Dictionary<Guid, List<AvailabilityWindow>> BuildTeacherAvailabilityWindows()
        {
            ...
            foreach (var teacher in _data.Teachers)
            {
                ...
                // First, collect all availability for this teacher grouped by day
                ...
                foreach (var dayGroup in availabilityByDay)
                {
                    ...
                    // Find the earliest start time for this teacher on this day
        ...
        private int AddStudentUnavailabilityConstraints(CpModel model, List<ClassToSchedule> classes, Dictionary<Guid, ClassVariables> classVars)
        {
            ...
            foreach (var cls in classes)
            {
                foreach (var altVar in classVars[cls.Id].AlternativeVars)
                {
                    ...
                    // For each student in this class, check their unavailability
                    foreach (var studentId in cls.StudentIds)
                    {
                        ...
                        {
                            // Only check unavailability on the same day as this alternative
                            ...
                            // Check if unavailability overlaps with this window at all
                            if (unavailEnd <= alt.WindowStartMinutes || unavailStart >= alt.WindowEndMinutes)
                            {
                                // No overlap with window, no constraint needed
                                continue;
                            }

                            // Convert unavailability to window-relative coordinates
                            ...
                            // Class interval is [Start, Start + Duration)
                            // Unavailability is [relativeUnavailStart, relativeUnavailEnd)
                            // No overlap means: Start + Duration <= relativeUnavailStart OR Start >= relativeUnavailEnd

                            // If this alternative is selected, class must be before or after the unavailability
                            var isBefore = model.NewBoolVar($"before_unavail_{cls.Id}_{alt.Day}_{unavailStart}_{studentId}");

                            // If isBefore: class ends before unavailability starts
                            model.Add(altVar.Start + cls.DurationMinutes <= relativeUnavailStart)
                                .OnlyEnforceIf([altVar.IsPresent, isBefore]);

                            // If !isBefore: class starts after unavailability ends
        ...
        /// <summary>
        /// Alpha objective: Maximize free time at end of teacher's day.
        /// We minimize the end time relative to the teacher's day start, which pushes classes earlier in their day.
        /// This ensures fairness: a teacher starting at 2pm isn't penalized compared to one starting at 9am.
        /// </summary>
        private void AddFreeTimeObjective(CpModel model, List<ClassToSchedule> classes, Dictionary<Guid, ClassVariables> classVars, List<LinearExpr> objectiveTerms)
        {
            ...
            foreach (var cls in classes)
            {
                foreach (var altVar in classVars[cls.Id].AlternativeVars)
                {
                    // Calculate the offset from teacher's day start to this window's start
                    var windowOffset = altVar.Alternative.WindowStartMinutes - altVar.Alternative.TeacherDayStartMinutes;
                    
                    // The penalty is: (window_start - teacher_day_start) + rel_end
                    // This equals: abs_end - teacher_day_start (end time relative to teacher's day)
                    ...
                    // endPenalty = windowOffset + altVar.End (when selected)
        ...
        /// <summary>
        /// Beta objective: Minimize gaps between a student's classes.
        /// Same-day classes are penalized by the gap between them (using absolute times).
        /// Cross-day classes are penalized by the CrossDayPenalty.
        /// Includes both flexible classes and fixed groups.
        /// </summary>
        ...
        /// <summary>
        /// Gamma objective: Schedule younger students earlier in the teacher's day.
        /// Penalty is inversely proportional to age, multiplied by start time relative to teacher's day.
        /// </summary>
        private void AddAgePriorityObjective(CpModel model, List<ClassToSchedule> classes, Dictionary<Guid, ClassVariables> classVars, List<LinearExpr> objectiveTerms)
        {
            ...
            foreach (var cls in classes)
            {
                ...
                foreach (var altVar in classVars[cls.Id].AlternativeVars)
                {
                    ...
                    // The penalty is based on: (window_start - teacher_day_start) + rel_start
                    // This equals: abs_start - teacher_day_start (start time relative to teacher's day)
                    ...
                    // penalty = ageFactor * (windowOffset + altVar.Start) when selected
        ...
        private class ClassAlternative
        {
            ...
            /// <summary>
            /// The earliest time this teacher is available on this day.
            /// Used for calculating teacher-day relative times in objectives.
            /// </summary>
